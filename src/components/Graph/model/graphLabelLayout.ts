...
export function layoutGraphLabels(
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    labelWidths.push(getGraphLabelWidth(node.label));
  }
  for (let index = 0; index < obstacleNodes.length; index += 1) {
    const node = obstacleNodes[index]!;
    collisionIndex.insert(
      getGraphNodeBounds(node, viewport, priorityIdSet.has(node.id)),
      nodeIndexById.get(node.id) ?? nodes.length + index,
    );
  }
  ...
  for (const id of priorityIds) {
    ...
    if (index !== undefined) priorityNodes.push(nodes[index]!);
  }
  const remainingNodes = nodes
    .filter((node) => !priorityIdSet.has(node.id))
